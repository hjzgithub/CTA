...
# 遍历各个品种
def get_portfolio(num_good, sample_name):
    ...
    for column in FACTORS.columns[1:-1:3]:
        name = column[6:]
        ...
        if win_times > 0 and lose_times > 0:
            ...
            if pnl_ratio > num_good:
                good_list.append(name)
        elif win_times > 0 and lose_times == 0:
            good_list.append(name)
    ...
    return good_list, times_list, win_rate_list, pnl_ratio_list, cash_all, guarantee_total
...
def result_analysis(num_good, sample_name=0):
    good_list, times_list, win_rate_list, pnl_ratio_list, cash_all, guarantee_total = get_portfolio(num_good, sample_name)
    ...
    end_date = FACTORS['date'].iloc[-2] if FACTORS['date'].iloc[-2] <= DATA['date'].iloc[-2] else DATA['date'].iloc[-2]
